...
|---|---|---|---|
| **User** | `name`, `email`, `image` (avatar opcional), `role` (`user`/`creator`/`admin`), `banned`, `createdAt` | Tiene perfil (`UserProfile`), puede crear artistas y eventos | Es quien se loguea. Email es único. |
| **UserProfile** | `status` (`PENDING`/`ACTIVE`/`BLOCKED`), `bio` | Pertenece a un User | El status determina el acceso al sitio. Nuevas cuentas arrancan en `PENDING`. |
...
- **Botón "Crear evento" del dashboard del creator:** funcionalmente está roto hoy (un bug de role-check redirige al creator al `/dashboard` sin feedback). El rediseño no resuelve el bug, pero conviene que el CTA del onboarding lleve a un flujo que sí funcione una vez parcheado.
